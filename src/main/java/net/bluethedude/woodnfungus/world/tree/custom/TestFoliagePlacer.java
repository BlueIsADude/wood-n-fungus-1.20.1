...
public class TestFoliagePlacer extends FoliagePlacer {
    public static final Codec<TestFoliagePlacer> CODEC = RecordCodecBuilder.create(testFoliagePlacerInstance ->
            fillFoliagePlacerFields(testFoliagePlacerInstance).and(Codec.intRange(0, 12).fieldOf("height")
                    .forGetter(instance -> instance.height)).apply(testFoliagePlacerInstance, TestFoliagePlacer::new));

    private int height;

    public TestFoliagePlacer(IntProvider radius, IntProvider offset, int height) {
        super(radius, offset);
    }

    @Override
    protected FoliagePlacerType<?> getType() {
        return ModFoliagePlacerTypes.TEST_FOLIAGE_PLACER;
    }
