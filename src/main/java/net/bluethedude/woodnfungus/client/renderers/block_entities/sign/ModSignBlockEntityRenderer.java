package net.bluethedude.woodnfungus.client.renderers.block_entities.sign;
...
public class ModSignBlockEntityRenderer<T extends ModSignBlockEntity> implements BlockEntityRenderer<T> {
    public static final EntityModelLayer LAYER_LOCATION = new EntityModelLayer(new Identifier(WoodNFungus.MOD_ID, "sign/palm"), "main");
    ...
    public ModSignBlockEntityRenderer(BlockEntityRendererFactory.Context ctx) {
        ...
        typeToModel = ImmutableMap.of(ModWoodTypes.PALM, new ModSignBlockEntityRenderer.SignModel(ctx.getLayerModelPart(LAYER_LOCATION)));
    }
    ...
    public void render(T signBlockEntity, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay) {
        ...
        ModSignBlockEntityRenderer.SignModel signModel = typeToModel.get(ModWoodTypes.PALM);
        signModel.stick.visible = blockState.getBlock() instanceof SignBlock;
        this.render(signBlockEntity, matrices, vertexConsumers, light, overlay, blockState, abstractSignBlock, ModWoodTypes.PALM, signModel);
    }
    ...
    SpriteIdentifier getTextureId(WoodType signType) {
        return new SpriteIdentifier(TexturedRenderLayers.SIGNS_ATLAS_TEXTURE, new Identifier(WoodNFungus.MOD_ID,"entity/signs/palm"));
    }
